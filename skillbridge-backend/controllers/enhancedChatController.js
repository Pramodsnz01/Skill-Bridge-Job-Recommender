...
function getEnhancedResponse(userMessage, userContext = null) {
...
const enhancedChatController = {
  ...
  async sendMessage(req, res) {
    try {
      ...
      // Get base response
      const baseResponse = getEnhancedResponse(message);
  ...
  async getChatHistory(req, res) {
    try {
      const { userId, limit = 50, sessionId } = req.query;
